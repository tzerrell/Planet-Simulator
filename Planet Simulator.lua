...
function MapConstants:New()
	...
	--directions
		...
		--world age
		mconst.RANDOMAGEWORLD = 0
		mconst.YOUNGWORLD = 1
		mconst.MIDDLEAGEDWORLD = 2
		mconst.OLDWORLD = 3
		
		--world temperature
		mconst.RANDOMTEMPERATUREWORLD = 0
		mconst.COLDWORLD = 1
		mconst.TEMPERATEWORLD = 2
		mconst.HOTWORLD = 3
		
		--world rainfall
		mconst.RANDOMRAINFALLWORLD = 0
		mconst.ARIDWORLD = 1
		mconst.NORMALRAINFALLWORLD = 2
		mconst.WETWORLD = 3
		
		--world sea level
		mconst.RANDOMSEALEVELWORLD = 0
		mconst.LOWSEAWORLD = 1
		mconst.MEDIUMSEAWORLD = 2
		mconst.HIGHSEAWORLD = 3
		
		--world resources
		mconst.RANDOMRESOURCESWORLD = 0
		mconst.SPARSERESOURCESWORLD = 1
		mconst.STANDARDRESOURCESWORLD = 2
		mconst.ABUNDANTRESOURCESWORLD = 3
		mconst.LEGENDARYSTARTRESOURCESWORLD = 4
		mconst.STRATEGICBALANCERESOURCESWORLD = 5
		
		--world map preset
		mconst.PANGAEAWORLD = 1
		mconst.CONTINENTALWORLD = 2
		--mconst.TERRAWORLD = 3
		
		--world start placement
		mconst.STARTANYWHERE = 1
		mconst.ALLCIVSONLARGESTCONTINENT = 2
		
		--world coastal waters
		mconst.RANDOMCOASTALWATERS = 0
		mconst.LANDADJACENTCOASTS = 1
		mconst.VERYNARROWCOASTS = 2
		mconst.NARROWCOASTS = 3
		mconst.STANDARDCOASTS = 4
		mconst.WIDECOASTS = 5
		mconst.VERYWIDECOASTS = 6
		mconst.SUPERFLUOUSLYWIDECOASTS = 7
		
		--world island style
		mconst.RANDOMISLANDS = 0
		mconst.MINIMALISLANDS = 1
		mconst.SCATTEREDANDSMALLISLANDS = 2
		mconst.LARGEANDINFREQUENTISLANDS = 3
		mconst.ARCSOFSMALLISLANDS = 4
...
-------------------------------------------------------------------------------------------
function MapConstants:InitializeMapScriptInfoChoices()
	--[[ This function reads the custom map options generated by GetMapScriptInfo and
	saves them as variables in the MapConstants class with sensible variable names
	and enumerative constant values (rather than having magic numbers for both, which
	is how they go into and come out of the Map.GetCustomOption function). 
	
	To retain the clear naming and simpler menu reorganizing this gains us, don't
	call Map.GetCustomOption directly outside of this function; instead use the
	corresponding variable from the appropriate MapConstants (currently the only
	MapConstants is "mc"). If you add menu items, add enums in MapConstants:New()
	for the values it can take, and add a chunk of code here that sets an appropriate
	variable in MapConstants to one of these enumerated values based on what
	it reads from Map.GetCustomOption ]]
	--Age
	...
	if age == 1 then
		self.worldAge = self.YOUNGWORLD
	elseif age == 2 then
		self.worldAge = self.MIDDLEAGEDWORLD
	elseif age == 3 then
		self.worldAge = self.OLDWORLD
	else
		print(string.format("Warning: Invalid world age option %x - Planet Simulator", age))
		print("Defaulting to Middle Aged World - Planet Simulator")
		self.worldAge = self.MIDDLEAGEDWORLD
	end
	...
	if temp == 1 then
		self.worldTemp = self.COLDWORLD
	elseif temp == 2 then
		self.worldTemp = self.TEMPERATEWORLD
	elseif temp == 3 then
		self.worldTemp = self.HOTWORLD
	else
		print(string.format("Warning: Invalid world temperature option %x - Planet Simulator", temp))
		print("Defaulting to Temperate World - Planet Simulator")
		self.worldTemp = self.TEMPERATEWORLD
	end
	...
	if rain == 1 then
		self.worldRain = self.ARIDWORLD
	elseif rain == 2 then
		self.worldRain = self.NORMALRAINFALLWORLD
	elseif rain == 3 then
		self.worldRain = self.WETWORLD
	else
		print(string.format("Warning: Invalid world rainfall option %x - Planet Simulator", rain))
		print("Defaulting to Standard Rainfall World - Planet Simulator")
		self.worldRain = self.NORMALRAINFALLWORLD
	end
	...
	if sea == 1 then
		self.worldSeaLevel = self.LOWSEAWORLD
	elseif sea == 2 then
		self.worldSeaLevel = self.MEDIUMSEAWORLD
	elseif sea == 3 then
		self.worldSeaLevel = self.HIGHSEAWORLD
	else
		print(string.format("Warning: Invalid world sea level option %x - Planet Simulator", sea))
		print("Defaulting to Medium Sea Level World - Planet Simulator")
		self.worldSeaLevel = self.MEDIUMSEAWORLD
	end
	...
	if width == 1 then
		self.worldCoastWidth = self.LANDADJACENTCOASTS
	elseif width == 2 then
		self.worldCoastWidth = self.VERYNARROWCOASTS
	elseif width == 3 then
		self.worldCoastWidth = self.NARROWCOASTS
	elseif width == 4 then
		self.worldCoastWidth = self.STANDARDCOASTS
	elseif width == 5 then
		self.worldCoastWidth = self.WIDECOASTS
	elseif width == 6 then
		self.worldCoastWidth = self.VERYWIDECOASTS
	elseif width == 7 then
		self.worldCoastWidth = self.SUPERFLUOUSLYWIDECOASTS
	else
		print(string.format("Warning: Invalid coastal width option %x - Planet Simulator", width))
		print("Defaulting to Standard Width Coasts - Planet Simulator")
		self.worldCoastWidth = self.STANDARDCOASTS
	end
	...
	if isles == 1 then
		self.worldIslandType = self.MINIMALISLANDS
	elseif isles == 2 then
		self.worldIslandType = self.SCATTEREDANDSMALLISLANDS
	elseif isles == 3 then
		self.worldIslandType = self.LARGEANDINFREQUENTISLANDS
	elseif isles == 4 then
		self.worldIslandType = self.ARCSOFSMALLISLANDS
	elseif isles == 5 then
		self.worldIslandType = self.FREQUENTANDVARIEDISLANDS
	else
		print(string.format("Warning: Invalid world islands option %x - Planet Simulator", isles))
		print("Defaulting to Scattered and Small Islands - Planet Simulator")
		self.worldIslandType = self.SCATTEREDANDSMALLISLANDS
	end
end
-------------------------------------------------------------------------------------------
...
function MapConstants:InitializeWorldAge()
	...
	elseif self.worldAge == self.MIDDLEAGEDWORLD then	--Standard (Default)
		...
		print(string.format("Warning: Invalid mc.worldAge value %x", self.worldAge))
	end
end
-------------------------------------------------------------------------------------------
function MapConstants:InitializeTemperature()
	if self.worldTemp == self.COLDWORLD then						--Cold
	...
	elseif self.worldTemp == self.HOTWORLD then					--Warm
	...
	elseif self.worldTemp == self.TEMPERATEWORLD then				--Temperate (default)
		...
		print(string.format("Warning: Invalid mc.worldTemp value %x", self.worldTemp))
	end
end
-------------------------------------------------------------------------------------------
function MapConstants:InitializeRainfall()
	if self.worldRain == self.ARIDWORLD then				--Arid
	...
	elseif self.worldRain == self.WETWORLD then				--Wet
	...
	elseif self.worldRain == self.NORMALRAINFALLWORLD then	--Standard
		...
		print(string.format("Warning: Invalid mc.worldRain value %x", self.worldRain))
	end
	--TODO: Remove Debug
	--self.riverPercent = 0.99
	--self.minRiverSize = 1
end
-------------------------------------------------------------------------------------------
function MapConstants:InitializeSeaLevel()
	if self.worldSeaLevel == self.LOWSEAWORLD then			--Low
		print("Setting low sea level constants - Planet Simulator")
		self.landPercent = 0.37 
	elseif self.worldSeaLevel == self.HIGHSEAWORLD then		--High
		print("Setting high sea level constants - Planet Simulator")
		self.landPercent = 0.25		
	elseif self.worldSeaLevel == self.MEDIUMSEAWORLD then		--Medium (default)
		print("Setting medium sea level constants - Planet Simulator")
		self.landPercent = 0.31 --Percent of land tiles on the map.
	else
		print(string.format("Warning: Invalid mc.worldSeaLevel value %x", self.worldSeaLevel))	
	end
end
-------------------------------------------------------------------------------------------
function MapConstants:InitializeCoasts()
	...
	elseif self.worldCoastWidth == 4 then	--Standard (Default)
		...
		print(string.format("Warning: Invalid mc.worldCoastWidth value %x", self.worldCoastWidth))	
	end
end
-------------------------------------------------------------------------------------------
...
function MapConstants:InitializeIslands()
	...
	if self.worldIslandType == self.MINIMALISLANDS then					--Minimal
	...
	elseif self.worldIslandType == self.SCATTEREDANDSMALLISLANDS then	--Scattered and Small
		print("Setting scattered and small islands constants - Planet Simulator")
		self.hotspotFrequency = 0.2
		self.oceanicVolcanoFrequency = 0.02
		self.islandExpansionFactor = 0
	elseif self.worldIslandType == self.LARGEANDINFREQUENTISLANDS then	--Large and Infrequent
	...
	elseif self.worldIslandType == self.FREQUENTANDVARIEDISLANDS then	--Frequent and Varied
	...
	elseif self.worldIslandType == self.ARCSOFSMALLISLANDS then			--Arcs of Small Islands
		...
		print(string.format("Warning: Invalid mc.worldIslandType value %x", self.worldIslandType))
	end
end
-------------------------------------------------------------------------------------------
function MapConstants:ReverseFlow(dir)
	if dir == self.NOFLOW then
		print("Warning: Reversing NOFLOW")
		return self.NOFLOW
	elseif dir == self.WESTFLOW then
		return self.EASTFLOW
	elseif dir == self.EASTFLOW then
		return self.WESTFLOW
	elseif dir == self.VERTFLOW then
		return self.VERTFLOW
	else
		print("Warning: Invalid direction for ReverseFlow. Returning input.")
		return dir
	end
end
-------------------------------------------------------------------------------------------
...
function RiverMap:SetFlowDestinations()
	...
	for n=1,#junctionList do
		...
		if #validList > 0 then
			...
			self:GetJunctionNeighbor(junction.flow, junction).tributaries:add(mc:ReverseFlow(junction.flow)) --Keeps track of tributaries to each junction.
		else
...
-------------------------------------------------------------------------------------------
function RiverMap:GetFlowSize(junction)
	--LL
	if junction.inflowsSet then
	...
	junction.size = junction.inflowTable[mc.NOFLOW]
	for tributeDir,_ in pairs(junction.tributaries) do
		junction.inflowTable[tributeDir] = self:GetFlowSize(self:GetJunctionNeighbor(tributeDir,junction))
		junction.size = junction.size + junction.inflowTable[tributeDir]
	end
	
	local minJunctionSize = 0.0001	--Not to be confused with minJunctionSize from GenerateRivers()
									--TODO: Load this from a map constant
...
-------------------------------------------------------------------------------------------
function RiverMap:addRiverSegment(junction,dir)
	--Returns the number of river segments added; either 0 if we failed to add the requested
	--segment, or 1 if it was successfully added
	
	local hex = nil
	if dir == mc.NOFLOW then
		print("Warning: Can't add river segment in NOFLOW direction.")
		return 0
	elseif dir == mc.EASTFLOW then
		if junction.isNorth then
			hex = self:GetRiverHexNeighbor(junction, false)
			if hex == nil then
			...
			if hexIndex == -1 then
				print("Warning: junction separated from hex in addRiverSegment")
				return 0
			end
	...
	elseif dir == mc.VERTFLOW then
		...
		if junction.isNorth then
			hex.WOfRiverFlow = FlowDirectionTypes.FLOWDIRECTION_NORTH
		else
			hex.WOfRiverFlow = FlowDirectionTypes.FLOWDIRECTION_SOUTH
		end
		return 1
	else
		print("Warning: Unexpected direction passed to addRiverSegment.")
		return 0
	end
end
-------------------------------------------------------------------------------------------
function RiverMap:extendUpstream(junction)
	--Do not use separately from extendDownstream (but you *can* use extendDownstream without
	--this function)
	--
	...
	local dir = nil
	local biggestSize = 0
	for k,_ in pairs(tributaries) do	-- Pick largest tributary
		local size = self:GetJunctionNeighbor(k,junction).size
		if size > biggestSize then
			dir = k
		end
	end
	if dir == nil then
		print("Warning: Empty tributaries in extendUpstream.")
		return 0
	end
	...
	primaryParent.downstreamFlowsConstructed = true --if we constructed rivers to the primaryParent from downstream, we must have constructed the downstream flows
	return 1 + self:extendUpstream(primaryParent)
end
-------------------------------------------------------------------------------------------
function RiverMap:extendDownstream(junction)
	--If junction is touching the ocean, is not flowing, is on the map edge, or is a junction we've
	--already extended downstream, halt; otherwise add a river segment from the junction in the 
	--direction of its flow, and if possible iterate this from the destination junction.
	--
	--Returns the number of river segments added before recursion halted.
	
	if self:IsTouchingOcean(junction) then	--If we're at ocean, we're done extending downstream
		return 0	
	end
	if junction.downstreamFlowsConstructed then	--If we've already extended downstream from here, we can stop
		return 0
	end
	junction.downstreamFlowsConstructed = true
	
	local hex = nil
	if self:addRiverSegment(junction,junction.flow) == 0 then
		return 0
	end	--else fall through
	
	local newJunction = self:GetJunctionNeighbor(junction.flow, junction)
	if newJunction == nil then	--Halt if river flows off map
		print("extendDownstream halting due to river flowing off map")
		return 1	--We have extended the river by 1 in this function call, but we don't continue
	end
	return 1 + self:extendDownstream(newJunction)
end
-------------------------------------------------------------------------------------------
...
function RiverMap:GetFlowDirections(x,y)
	local index = elevationMap:GetIndex(x,y)
	local hex = self.riverData[index]
	
	return hex.WOfRiverFlow, hex.NWOfRiverFlow, hex.NEOfRiverFlow
end
-------------------------------------------------------------------------------------------
function RiverMap:OldGetFlowDirections(x,y)
...
function RiverHex:New(x,y)
	...
	new_inst.WOfRiverFlow = FlowDirectionTypes.NO_FLOWDIRECTION
	new_inst.NWOfRiverFlow = FlowDirectionTypes.NO_FLOWDIRECTION
	new_inst.NEOfRiverFlow = FlowDirectionTypes.NO_FLOWDIRECTION
...
function RiverJunction:New(x,y,isNorth)
	...
	new_inst.size = 0.0
	--LL
	new_inst.tributaries = Set:New(nil)
	do	--define inflowTable, which keeps track of how much water comes from each flow direction
		new_inst.inflowTable = {}	--Named inflow because positive numbers represent amount flowing in; - numbers are amount flowing out
		new_inst.inflowTable[mc.NOFLOW] = 0 --Represents how much precipitation (+) or evaporation (-) is native to the tile
		new_inst.inflowTable[mc.EASTFLOW] = 0
		new_inst.inflowTable[mc.WESTFLOW] = 0
		new_inst.inflowTable[mc.VERTFLOW] = 0
	end
	new_inst.inflowsSet = false
	new_inst.downstreamFlowsConstructed = false
